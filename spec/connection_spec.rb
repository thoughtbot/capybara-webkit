...
describe Capybara::Webkit::Connection do
  ...
  [{}, {socket_class: Capybara::Webkit::SocketDebugger}].each do |options|
    context "Connection with options: #{options}" do
      ...
      it "raises an error if the server has stopped", skip_on_windows: true do
        ...
        expect { Capybara::Webkit::Connection.new options}
          .to raise_error(
            Capybara::Webkit::ConnectionError, "#{path} failed to start."
          )
      end
      ...
      it 'does not forward stderr to nil' do
        ...
        Capybara::Webkit::Connection.new(options.merge(:stderr => nil))
      end
      ...
      it 'prints a deprecation warning if the stdout option is used' do
        ...
        Capybara::Webkit::Connection.new(options.merge(:stdout => nil))
      end
      ...
      it 'sets appropriate options on its socket' do
        ...
        Capybara::Webkit::Connection.new options
      end

      it "chooses a new port number for a new connection" do
        new_connection = Capybara::Webkit::Connection.new options
